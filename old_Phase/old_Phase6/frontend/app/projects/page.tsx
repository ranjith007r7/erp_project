  ...
  return (
    <main className="min-h-screen p-8">
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        <form onSubmit={addProject} className="bg-white rounded-xl shadow-sm p-4 space-y-2">
          <h2 className="font-semibold text-slate-700 text-sm">New Project</h2>
          <input
        ...
        <form onSubmit={addTask} className="bg-white rounded-xl shadow-sm p-4 space-y-2">
          <h2 className="font-semibold text-slate-700 text-sm">New Task</h2>
          <select
            required
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {projects.map((project) => (
          <section key={project.id} className="bg-white rounded-xl shadow-sm p-4">
            <h2 className="font-semibold text-slate-800 mb-3">{project.name} <span className="text-xs text-slate-400">({project.status})</span></h2>
            <div className="space-y-2">
              {tasks.filter((t) => t.project_id === project.id).map((task) => (
                <div key={task.id} className="border border-slate-200 rounded-lg p-2 text-sm">
                  <div className="flex justify-between items-center">
                    <label className="flex items-center gap-2">
                      <input type="checkbox" checked={task.status === "done"} onChange={() => toggleTaskDone(task)} />
                      <span className={task.status === "done" ? "line-through text-slate-400" : "text-slate-800"}>
                        {task.title}
                      </span>
                    </label>
                    <span className="text-xs text-slate-400">{task.priority} · {hoursFor(task.id)}h logged</span>
                  </div>
